...
GameView::GameView(QWidget *parent)
        : QGraphicsView(parent), m_gameViewModel(nullptr), m_scene(new QGraphicsScene(this))
{
    // 设置场景的大小
...
void GameView::setGameViewModel(GameViewModel *viewModel)
{
...
void GameView::createBoard(int sizeX, int sizeY)
{
    ...
    for (int y = 0; y < sizeY; ++y) {
        for (int x = 0; x < sizeX; ++x) {
            ...
            block->setBrush(Qt::magenta); // 默认颜色
